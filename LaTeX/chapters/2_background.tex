...
This master thesis will focus in the following chapters on the one-factor Hull–White model, a prominent Gaussian short rate model that enhances the classical Vasicek formulation by introducing a time-dependent drift term to ensure an exact fit to the initial term structure of interest rates.
...
Having outlined the diverse range of problems that neural networks are capable of addressing, it is now essential to examine their internal structure. Understanding the architecture of neural networks provides insight into how these models represent and process information to solve such complex tasks.
...
While the architecture defines the structural capacity of a neural network, its practical effectiveness ultimately depends on how well the model’s parameters are optimized. This leads to the central concept of learning—the process by which neural networks iteratively adjust their weights to minimize prediction errors and improve performance.
...
\textcite{hernandez2016model} investigates the use of artificial neural networks (ANNs) as a fast alternative to traditional optimization methods for calibrating the one-factor Hull–White model with constant parameters. The study compares the neural network approach with the conventional Levenberg–Marquardt optimization method, which served as the calibration benchmark.
...
A feed-forward neural network was then trained to approximate the mapping between market data and model parameters, thereby shifting the computational burden to an offline training phase. Once trained, the network could perform calibrations almost instantaneously.
Backtesting results demonstrated that the neural network achieved a calibration accuracy comparable to that of the Levenberg–Marquardt optimizer while reducing computation time by several orders of magnitude. Although performance gradually deteriorated after six to twelve months, periodic retraining every few months effectively restored accuracy. Overall, the study showed that neural networks can deliver substantial efficiency gains in interest rate model calibration without sacrificing precision.
