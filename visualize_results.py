...
"""
Standalone Script for Visualizing Hull-White Calibration Comparison Results
...
def generate_summary_tables(df: pd.DataFrame, save_dir: str):
    """Calculates, prints, and saves the three summary tables."""
    print("="*80)
...
def plot_rmse_distribution(df: pd.DataFrame, save_path: str):
    """Plot 2: Distribution of Daily RMSE"""
    df_melted = df.melt(
        id_vars=['Date'],
...
def plot_volatility_surface(df: pd.DataFrame, save_path_prefix: str):
    """Plot 4: Volatility Surface Reconstruction for a Representative Day"""
    if df.empty: return
...
def plot_error_heatmaps(df: pd.DataFrame, save_path: str):
    """Plot 5: Heatmap of Mean Prediction Errors"""
    df_copy = df.copy()
...
def plot_error_by_expiry(df: pd.DataFrame, save_path: str):
    """Plot 6: Error Distribution by Swaption Expiry"""
    df_copy = df.copy()
    df_copy['Expiry'] = df_copy['ExpiryStr'].apply(parse_tenor_to_years)
...
def plot_scatter_comparison(df: pd.DataFrame, save_path: str):
    """Plot 7: Scatter Plot of Model Volatility vs. Market Volatility"""
...
# <<< --- NEW PLOTS INTEGRATED BELOW --- >>>

def plot_error_by_tenor(df: pd.DataFrame, save_path: str):
    """Plot 11: Error Distribution by Swaption Tenor"""
    df_copy = df.copy()
    df_copy['Tenor'] = df_copy['TenorStr'].apply(parse_tenor_to_years)
...
def plot_error_by_volatility(df: pd.DataFrame, save_path: str):
    """Plot 12: Error Analysis by Volatility Level"""
    df_copy = df.copy()
